...
vec4 compute_lighting(vec3 direction, vec4 lightcolor, vec3 normal, vec3 halfvec, vec4 mydiffuse, vec4 myspecular, float myshininess) {
    const float n_dot_l = dot(normal, direction);
    const vec4 lambert = mydiffuse * lightcolor * max (n_dot_l, 0.0);
    const vec4 phong = myspecular * lightcolor * pow (max(dot(normal, halfvec), 0.0), myshininess);
...
void main (void) {
    ...
    const vec3 mypos = myvertex.xyz / myvertex.w;
    const vec3 eyedirn = normalize(eyepos - mypos);

    // Normalise the normal at that point.
    const vec3 normal = normalize(mynormal);

    // Start with ambient color, and iterate through each light source (on or off).
    vec4 finalcolor = vec4(ambient);
    for (int i = 0; i < num_lights; i++) {
        const vec4 light_position = inverse(modelview) * light_posn[i];
        const vec3 position = light_position.xyz / light_position.w;
        const vec3 direction = normalize(position - mypos);
        const vec3 half_i = normalize(direction + eyedirn);
        const vec4 light_color = light_col[i];
