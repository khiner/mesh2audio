...
Arrow::Arrow(float length, float base_radius, float tip_radius, float tip_length, uint segments) : Geometry() {
    ...
    for (uint i = 0; i < segments * 2; i += 2) {
        TriangleIndices.append({i, i + 1, i + 2, i + 1, i + 3, i + 2});
    }
    ...
    for (uint i = 0; i < segments; ++i) {
        TriangleIndices.append({base_cap_center_index, 2 * i + 1, 2 * ((i + 1) % segments) + 1});
    }
    ...
    // Tip indices.
    for (uint i = 0; i < segments; ++i) {
        TriangleIndices.append({base_vertex_count + i, base_vertex_count + i + 1, base_vertex_count + segments + 1});
    }
}
